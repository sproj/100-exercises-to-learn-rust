...
#[derive(Debug)]
enum TicketNewError {
    InvalidTitle(InvalidInputError),
    InvalidDescription(InvalidInputError),
}

#[derive(Debug)]
enum InvalidInputError {
    Empty { message: &'static str },
    TooLong { message: &'static str },
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(t) => t,
        Err(TicketNewError::InvalidDescription(e)) => {
            Ticket::new(title, "Description not provided".to_string(), status).expect("Should never be invoked")
        }
        Err(e) => panic!("{:?}", e),
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::InvalidTitle(InvalidInputError::Empty {
                message: "Title cannot be empty",
            }));
        }
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle(InvalidInputError::TooLong {
                message: "Title cannot be longer than 50 bytes",
            }));
        }
